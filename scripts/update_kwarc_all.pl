...
"cd ~/LaTeXML; git stash; git pull --rebase; git stash pop; rm -rf blib/; cpanm --uninstall LaTeXML.pm -f; cpanm . --notest; ".
"cd ~/LaTeXML-Plugin-Cortex; git stash; git pull --rebase; git stash pop; rm -rf blib/; cpanm --uninstall LaTeXML.pm -f; cpanm .; ".
